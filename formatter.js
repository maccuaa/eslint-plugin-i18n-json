...
const formatter = (results) => {
    ...
    .map(({ filePath, messages: fileMessages, errorCount: fileErrorCount, warningCount: fileWarningCount }) => {
      ...
      const formattedFileMessages = fileMessages
        .map(({ ruleId, severity, message }) => {
          let messageHeader =
            severity === 1
              ? `${logSymbols.warning} ${chalk.inverse.yellow(" WARNING ")}`
  ...
  const totalErrorsCountFormatted = `${chalk.bold.red(">")} ${logSymbols.error} ${chalk.bold.red(
    totalErrorsCount
  )} ${chalk.bold.red(plur("ERROR", totalErrorsCount))}`;
  const totalWarningsCountFormatted = `${chalk.bold.yellow(">")} ${logSymbols.warning} ${chalk.bold.yellow(
    totalWarningsCount
  )} ${chalk.bold.yellow(plur("WARNING", totalWarningsCount))}`;
